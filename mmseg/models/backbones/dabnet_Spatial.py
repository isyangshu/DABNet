...
class SIM(nn.Module):
    ...
    def forward(self, x):
        ...
        x_patch = rearrange(x, 'b c (p1 h) (p2 w) -> b (p1 p2 c) h w', h=2, w=2)  # N, C*H*W//4, 2, 2

        # first conv
        x_h = self.relu(self.bnh_0(self.h2h_0(x)))  # N, C_, H, W
        x_l = rearrange(self.avg(x_patch), 'b (p1 p2 c) h w -> b c (p1 h) (p2 w)', p1=H//2, p2=W//2).contiguous()  # N, C, H//2, W//2
        x_l = self.relu(self.bnl_0(self.h2l_0(x_l)))  # N, C_, H, W
...
class DABNet_Spatial(BaseModule):
    """BiSeNetV1 backbone.
    ...
    def __init__(self,
        ...
        super(DABNet_Spatial, self).__init__(init_cfg=init_cfg)
