...
class SIM(nn.Module):
    def __init__(self, h_C, l_C, norm_layer='BN2d'):
        ...
        self.patch_size = 8
        self.h2l_pool = nn.AvgPool2d((self.patch_size, self.patch_size), stride=self.patch_size)
        self.l2h_up = cus_sample
        
        self.h2l_0 = nn.Conv2d(h_C*self.patch_size*self.patch_size, l_C, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        x_patch_1 = rearrange(x, 'b c (h p1) (w p2) -> b (c p1 p2) h w', p1=self.patch_size, p2=self.patch_size)  # N, 4*C, H//2, W//2
        x_patch_2 = rearrange(x, 'b c (p1 h) (p2 w) -> b (p1 p2 c) h w', h=self.patch_size, w=self.patch_size)  # N, C*H*W//4, 2, 2

        # first conv
        x_h = self.relu(self.bnh_0(self.h2h_0(x)))  # N, C_, H, W
        x_l_1 = self.relu(self.bnl_0(self.h2l_0(x_patch_1)))  # N, C_, H, W
        x_l_2 = rearrange(self.avg(x_patch_2), 'b (p1 p2 c) h w -> b c (p1 h) (p2 w)', p1=H//self.patch_size, p2=W//self.patch_size).contiguous()  # N, C, H//2, W//2
...
class DABNet_P8(BaseModule):
    ...
    def __init__(self,
        ...
        super(DABNet_P8, self).__init__(init_cfg=init_cfg)
        ...
        self.SIM_2 = SIM(256, 64, norm_name)
        self.SIM_3 = SIM(512, 64, norm_name)
